    ...
    <nav className="fixed top-0 left-0 right-0 z-50 backdrop-blur-lg bg-background/80 border-b border-border">
      <div className="max-w-[1500px] mx-auto px-5 sm:px-8 lg:px-12">
        <div className="flex items-center justify-between h-[4.25rem]">
          ...
          <div className="hidden lg:flex items-center gap-1 overflow-x-auto whitespace-nowrap max-w-[82vw]">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                onClick={() => onTabChange(tab.id)}
                className="relative px-3 xl:px-4 py-2 text-base font-medium transition-all hover:scale-[1.02]"
      ...
      {mobileMenuOpen && (
        <motion.div
          ...
          className="lg:hidden bg-card border-t border-border max-h-[75vh] overflow-y-auto"
        >
